...
void plot_histos_group_averages() {
    ...
    std::vector<std::vector<TGraphErrors*>> graphs(3, std::vector<TGraphErrors*>(numSubDirs));

    for (int p = 0; p < 3; ++p) {
        for (int s = 0; s < numSubDirs; ++s) {
            graphs[p][s] = new TGraphErrors();
            graphs[p][s]->SetTitle(Form("%s vs. %s;<Npart>;%s átlaga",
                                        paramNames[p].c_str(),
                                        subdirTitles[s].c_str(),
                                        paramNames[p].c_str()));
            graphs[p][s]->SetMarkerStyle(20);
            graphs[p][s]->SetMarkerSize(1.2);
            graphs[p][s]->SetLineWidth(2);
        }
    }
    ...
    for (int groupIdx = 0; groupIdx < numGroups; ++groupIdx) {
        ...
        for (int subIdx = 0; subIdx < numSubDirs; ++subIdx) {
            ...
            double x = xValues[groupIdx];

            graphs[0][subIdx]->SetPoint(groupIdx, x, Navg);
            graphs[0][subIdx]->SetPointError(groupIdx, 0, NerrAvg);

            graphs[1][subIdx]->SetPoint(groupIdx, x, Ravg);
            graphs[1][subIdx]->SetPointError(groupIdx, 0, RerrAvg);

            graphs[2][subIdx]->SetPoint(groupIdx, x, Aavg);
            graphs[2][subIdx]->SetPointError(groupIdx, 0, AerrAvg);
        }
    }

    // Kirajzolás és mentés
    for (int p = 0; p < 3; ++p) {
        for (int s = 0; s < numSubDirs; ++s) {
            TCanvas* c = new TCanvas(Form("c%d_%d", p, s), graphs[p][s]->GetTitle(), 800, 600);
            graphs[p][s]->GetXaxis()->SetTitle("<Npart>");
            graphs[p][s]->GetYaxis()->SetTitle(yAxisLabels[p].c_str());

            if (p == 2) {
                graphs[p][s]->GetYaxis()->SetRangeUser(1.0, 2.0);
            }

            graphs[p][s]->Draw("AP");

            // fájlnév pl: plots/alpha_0_02.png
            std::string filename = "plots/" + paramNames[p] + "_" + subdirShort[s] + ".png";
